...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\nWelcome to group *${mdata.subject}* Jangan lupa intro kau!...Betah-betah yah di sini🖤(Untuk yang cwek njay)`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const vhtearkey = '7728hhgsYRTtjah2837'
			...
			mess = {
				wait: '_*Sabar..*_',
				success: '*Anjay Success!*',
				error: {
					stick: '*Itu video Njir*',
					Iv: '*Linknya mokad:v*',
					bug: '*Dih ngebug:v*'
				},
				only: {
			...
			const ownerNumber = ["6282157581762@s.whatsapp.net"] // ganti nomer lu
			...
			client.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: {"extendedTextMessage":{"text": `${target2}`,"textArgb":4294967295,"backgroundArgb":4280563264,"font":"SANS_SERIF","previewType":"NONE"}}}})
			}
			...
			const clientHide = (dari, teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? client.sendMessage(dari, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : client.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
			...
			switch(command) {
				case 'hidetag':
					...
					break
case 'togif':
	if (!isQuotedSticker) return client.sendMessage(from, "Ini Bukan Stiker Mank:v", text, { quoted: mek })

	client.sendMessage(from, "Tunggu Bentar Mank!", text, { quoted: mek })

	encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo

	media = await client.downloadAndSaveMediaMessage(encmedia)

	exec(`./webp2gif ${media} output.gif && ffmpeg -i output.gif output.mp4 -y`, (err) => {

		fs.unlinkSync(media)

		fs.unlinkSync("output.gif")

		if (err) return client.sendMessage(from, "Terjadi Kesalahan Fatal", text, { quoted: mek })

		const ann = fs.readFileSync("output.mp4")

		client.sendMessage(from, ann, video, {mimetype: Mimetype.gif, caption: "Sorry Device Kentang Jadi Glitch :v"})
	})
		break
case 'virtex':
...
case 'ytmp3':
				...
				buffer = await getBuffer(mp3.album)
				lagu = await getBuffer(mp3.download_audio)
				client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${mp3.title}.mp3`, quoted: mek, ptt: true})
			...
			break
case 'fakeimg':
            var konthol = body.slice(9)
            var fake1 = konthol.split("|")[0];
            var fake2 = konthol.split("|")[1];
            var bto = fs.readFileSync(`./src/image/${fake1}.jpeg`)
            var ato = fs.readFileSync(`./src/image/${fake2}.jpeg`)
            var option = {
              "thumbnail": ato}
            client.sendMessage(from, bto, "imageMessage", option)
         break
case 'return':
...
case 'silverbutton':
			...
			case 'goldbutton':
					...
					if (args.length < 1) return reply('Teksnya mana um')
					reply(mess.wait)
					anu = await fetchJson(`https://api.zeks.xyz/api/gplaybutton?text=${gol3}&apikey=apivinz`, {method: 'get'})
					gools2 = await getBuffer(anu.result)
					client.sendMessage(from, gools2, image, {quoted: mek})
					break
				case 'brainly':
				...
				client.sendMessage(from, `_Sabar Nyet!_`, MessageType.text, tahta)
...
case 'waifu':
	...
	break
case 'me':
  var wkwkwk = body.slice(3)
  var diriku = `${numbernye}@s.whatsapp.net`
	var falsucuy = `${fake}`
	// var taged = mek.message.extendedTextMessage.contextInfo.mentionedJid[0]
	const kontol = {
	contextInfo: {
	...
	client.sendMessage(from, wkwkwk, text, kontol)
	console.log('Berhasil fake reply ✓')
	break
case 'me2':
  var tekss = body.slice(4)
  	var itsme = `${numbernye}@s.whatsapp.net`
  	var fakenjay = `${fake}`
client.sendMessage(from, tekss, text, {quoted: { key: { fromMe: false, participant: itsme, ...(from ? { remoteJid: "6285791458996-1613050194@g.us" } : {}) }, message: { conversation: fakenjay }}})
console.log('Berhasil fake reply ✓')
break
case 'me3':
  var teksss = body.slice(4)
  var itsme = `${numbernye}@s.whatsapp.net`
  var palsu = `${fake}`
  client.sendMessage(from, teksss, text, {quoted: { key: { fromMe: false, participant: itsme, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { conversation: palsu }}})
  console.log('Berhasil fake reply ✓')
  break
case 'me4':
  var teksss = body.slice(4)
  var itsme = `${numbernye}@s.whatsapp.net`
  var palsu = `${fake}`
  client.sendMessage(from, teksss, text, {quoted: { key: { fromMe: false, participant: itsme, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { mimetype: "image/jpeg", "caption": palsu, "jpegThumbnail": fs.readFileSync('./src/image/thumbnail.jpeg') } } } } )
  console.log('Berhasil fake reply ✓')
  break
case 'cmd':
			reply(mess.wait)
			cmd = await getBuffer(`http://zekais-api.herokuapp.com/carbon?code=${body.slice(4)}`)
			client.sendMessage(from, cmd, image, {quoted:mek,caption:'*cmd*'})
			break
case 'oe':
  var wkwkwkwk = body.slice(3)
  var dirimu = `status@broadcast`
	var falsunjay = `${fake}`
	// var taged = mek.message.extendedTextMessage.contextInfo.mentionedJid[0]
	const kontuh = {
	contextInfo: {
	...
	client.sendMessage(from, wkwkwkwk, text, kontuh)
	break
case 'menu':
case 'help':
	runtime = process.uptime()
...
├ ${prefix}cekchat
│
└──「 *client-SELF*」`, MessageType.text, bruhhhh)
	break
case 'tomp3':
    client.updatePresence(from, Presence.composing) 
...
case 'term':
	...
	exec(cmd, (err, stdout) => {
	if(err) return client.sendMessage(from, `root@client315.0.0.20.8:~ ${err}`, text, { quoted: mek })
	if (stdout) {
